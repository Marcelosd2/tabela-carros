...
export const modeloCarro = Joi.object({
    nome: Joi.string().min(3).required().messages({
        'string.min': 'O nomw do carro deve ter pelo menos 3 caracteres.',
    ...
    velocidadeMaxima: Joi.number().min(1).required().messages({
        'number.min': 'A velocidade máxima dever ser maior ou igual a 1.',
        'any.required': 'A velocidade maxima é obrigatória',
    }), // Velocidade min. de 1
    consumo:Joi.number().min(0.1).required().messages({
        'number.min': 'o consumo deve ser ou igual a 0.1.',
        'any.required': 'O consumo é obrigatório',
    }),// Consumo mínimo de 0.1
    aceleracao: Joi.number().min(0).required().messages({
        'number.min': 'A aceleração deve ser ou igual a 0.1.',
        'any.required': 'A aceleração é obrigatório',
    }), // Aceleração min. de 0
    preco: Joi.number().min(0).required().messages({
        'number.min': 'O preço deve ser ou igual a 0.1.',
